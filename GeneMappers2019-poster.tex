...
\usepackage{wrapfig,anyfontsize}
...
Questions about inheritance and ancestry can be reframed as questions about the underlying tree sequence that represent our datasets:
          \begin{itemize}
          \item {\it Identity-by-descent: which samples share a common ancestor?}
...
\vspace{20cm}
             ...
             \block[titleoffsety=-2.5cm,bodyoffsety=-2.5cm]{4. Application: demography inference}{
...
Using \texttt{msprime} [1], we performed 50 000 simulations of each demographic scenario hypothesised in [2], and 20 simulations with fixed demographic parameter values that we took as the "truth". Each replicate contained 240 samples of a 100Mb chromosome with a recombination rate of $1.25\times 10^{-7}$ base/gen.
For each simulation, we calculated moments of IBD segment lengths at multiple time points.

We used the \texttt{abcrf} package to infer the most plausible demographic scenario, and the \texttt{abc} package with neural-net regression to estimate parameter values.\\

{\bf Results: inference of demographic scenario}
\begin{center}
\begin{tabularx}{.4\textwidth}{p{11cm}p{11cm}X}
\toprule
...
\begin{center}
\begin{tabularx}{.4\textwidth}{p{5cm}p{5cm}p{8cm}X}
\toprule
...
%%% BLOCK 5
 \block[titleoffsety=-3cm,bodyoffsety=-3cm]{5. Acknowledgements, references and further information}{
  ...
  \begin{columns}
 \begin{subcolumns}
    \subcolumn{25} \block[bodyoffsetx=1.5cm,bodyverticalshift=-5cm]{}{\small
