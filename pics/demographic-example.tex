...
\foreach \x in {0} \node[above,pop\x] (label\x) at ($(leftAx) + (pop\x) + (1, 10)$) {\tiny\bf Population \x};
\foreach \x in {1,2} \node[above,pop\x] (label\x) at ($(leftAx) + (pop\x) + (1, 5)$) {\tiny\bf Population \x};

% Times
\draw[very thin,color=gray] (12,0) node[below left] {\small Present day} -- (-1,0);
